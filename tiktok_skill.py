...
class TikTokSlideshowTool:
    ...
    def __init__(self):
        # Папка для збереження готових слайдів та файлу авторизації
    ...
    def save_to_drafts(self, selected_slide_numbers: list, post_description: str) -> str:
        """Завантажує картинки по одній у TikTok та зберігає як чернетку."""
        if not os.path.exists(self.state_file):
            return "ПОМИЛКА: Файл tiktok_state.json не знайдено. Переконайся, що користувач надіслав його."
        ...
        try:
            with sync_playwright() as p:
                ...
                # Завантажуємо сесію користувача
                ...
                # Перевірка на успішний логін
                if page.locator("input[type='file']").count() == 0:
                    debug_path = os.path.join(self.output_dir, "auth_error.png")
                    page.screenshot(path=debug_path)
                    browser.close()
                    return f"ПОМИЛКА ЛОГІНУ: Сесія злетіла або вимагається капча. ВІДПРАВ ФАЙЛ {debug_path} КОРИСТУВАЧУ."

                print(f"Починаємо послідовне завантаження {len(files)} слайдів...")
                
                # 1. Завантажуємо перший слайд у головний інпут
                page.locator("input[type='file']").first.set_input_files(files[0])
                time.sleep(12) # Чекаємо відкриття режиму редактора каруселі
                
                # 2. Якщо слайдів більше одного, додаємо інші через новий інпут
                if len(files) > 1:
                    for f in files[1:]:
                        page.locator("input[type='file']").last.set_input_files(f)
                        time.sleep(4) # Пауза для імітації людини
